...
/** Tooltip de gráficos con clases theme-aware (legible en claro y oscuro). */
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-md">
      {typeof label === 'string' && label !== '' && (
        <div className="mb-1 font-medium capitalize text-slate-700">{label}</div>
      )}
      {payload.map((p, i) => (
        <div key={i} className="flex items-center gap-2 text-slate-600">
          ...
          <span className="ml-3 tabular font-medium text-slate-800">{formatMoney(Number(p.value))}</span>
        </div>
      ))}
    </div>
  )
